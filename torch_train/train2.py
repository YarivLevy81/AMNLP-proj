...
from warmup_scheduler2 import WarmupScheduler2
...
def checkpoint(output_dir, writer, step, train_loss, model,
               device, eval_dataloader, optimizer,
    ...
    if args.save_best == 1 and update_best_f1 == 1:
        tf.print('Saving best f1')
        checkpoint_path = os.path.join(checkpoint_dir, f'best_f1.pt')
        torch.save(ckpt, checkpoint_path)

    if args.save_best == 1 and update_best_loss == 1:
        tf.print('Saving best loss')
        checkpoint_path = os.path.join(checkpoint_dir, f'best_loss.pt')
        torch.save(ckpt, checkpoint_path)
...
def main(args):
    ...
    optimizer.zero_grad()
    scheduler_wrap = WarmupScheduler2(optimizer=optimizer,
                                      warmup_steps=args.warmup_steps,
                                      init_lr=args.learning_rate,
                                      final_step=args.num_train_steps)
        ...
        for batch in train_dataloader:
            ...
            if steps % args.save_checkpoints_steps == 0:
                best_loss, best_f1 = checkpoint(
                    args.output_dir, writer, steps, loss.item() * args.gradient_accumulation_steps,
            ...
            if args.print_loss_steps and (steps + 1) % args.print_loss_steps == 0:
                ...
                tf.print(f"{timestamp} step: #{steps}, "
                      f"train_loss: {loss.item()*args.gradient_accumulation_steps}, lr: {optimizer.param_groups[0]['lr']}")
            steps += 1
